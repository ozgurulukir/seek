...
- `internal/source` — per-format parsers (code/claude/codex/markdown/images) plus `pdf.go`, which rasterizes PDF pages to PNG via `go-fitz` (bundled static MuPDF, cgo) and extracts text (embedded via `doc.Text`, or OCR via the `TextExtractor` interface for scanned pages). `source/parserdef/` holds the schema-driven parser engine (YAML schemas → SQLite/JSONL drivers) for opencode/copilot-cli/zed/hermes/zcode and schema-mode claude/codex; the JSONL driver also has a sliding-window mode (`messages_path` + `offset_field`) for model-I/O rollout logs like ZCode's (`~/.zcode/cli/rollout`), where each line carries a cumulative conversation window deduped by global index. Hermes Agent transcripts live in per-profile SQLite `state.db` files (`~/.hermes/state.db`, `~/.hermes/profiles/*/state.db`) — not JSONL.
