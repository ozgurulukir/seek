...
- **Text-bearing collections (all types)** — when `semantic.enabled: true` and
  the service is healthy, four fast fields generated by the optional local
  semantic tag service enrich every text-bearing collection type (markdown,
  code, conversations, pdf, documents, parser): `tags` (keyphrases + topic
  labels), `topics` (BERTopic), `entities` (`TYPE:Text` NER), `language`
  (detected ISO 639-1). All comma-joined and facetable via
  `--aggs <field>:terms`, and filterable via `--field`. Conversations, PDF,
  and documents are enriched during `seek sync`; markdown and code collections
  are covered by the backfill (`seek collection reindex <name>
  --semantic-only`). Without the service the fields are simply absent —
  enrichment degrades gracefully (WARN lines) and previously stored values are
  preserved.
